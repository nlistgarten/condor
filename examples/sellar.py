...
class Coupling(co.AlgebraicSystem):
    x = parameter(shape=3)
    y1 = variable(initializer=1.0, warm_start=warm_start)
    y2 = variable(initializer=1.0, warm_start=warm_start)

    residual(y1 == x[0] ** 2 + x[1] + x[2] - 0.2 * y2)
    residual(y2 == y1**0.5 + x[0] + x[1])


coupling = Coupling([5.0, 2.0, 1])  # evaluate the model numerically
...
for _ in range(2):
    ...
    print(
        "embedded solver iter count:",
        Sellar.coupling.implementation.callback.total_iters,
    )
